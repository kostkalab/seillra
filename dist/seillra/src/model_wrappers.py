...
class Sei_LLRA(nn.Module):
    def __init__(
        ...
        mode: Literal["sequence", "variant"] = "sequence",
        quant: Literal["CPU", "GPU_fp16", "GPU_int8", None] = None,
        compile: bool = False,
    ):
        super().__init__()
        self.quant = quant
    ...
    def set_mode(self, mode):
        if mode != "sequence" and mode != "variant":
            print(
                f"Mode options are: 'sequence' or 'variant'. Keeping current mode as {mode}"
            )
        else:
            if self.projection:
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            int8_layer.weight = bnb.nn.Int8Params(
                module.weight.data.detach().contiguous(), requires_grad=False
            )
            if module.bias is not None:
                int8_layer.bias = nn.Parameter(
                    module.bias.data.detach().clone(), requires_grad=False
                )
